...
build_bin() {
	...
	cmake_file=toolchain.cmake
	echo "set(CMAKE_SYSTEM_NAME Linux)" > ${cmake_file}
	echo "set(CMAKE_SYSTEM_PROCESSOR $6)" >> ${cmake_file}
	echo "set(CMAKE_C_COMPILER ${TOOLCHAIN}/bin/${COMPILER}-gcc)" >> ${cmake_file}
	echo "set(CMAKE_FIND_ROOT_PATH ${TOOLCHAIN}/${COMPILER})" >> ${cmake_file}
